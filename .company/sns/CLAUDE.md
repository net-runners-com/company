...
```bash
# テキスト直接指定
node .company/sns/threads/threads-post.js "投稿テキスト"

# 投稿ファイルから
node .company/sns/threads/threads-post.js --file posts/YYYY-MM-DD.md
```
...
詳細は `.company/skills/note-article/SKILL.md` を参照。
Threads詳細は `.company/skills/threads/SKILL.md` を参照。

```bash
# 記事生成
python3 .company/sns/note/scripts/generate_article.py --style ren

# 下書き投稿
python3 .company/sns/note/scripts/post_to_note.py "タイトル" article.md --profile "Profile 3"
```
