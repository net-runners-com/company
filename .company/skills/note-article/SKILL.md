...
| 種別 | パス |
|------|------|
| スクリプト（記事生成） | `.company/sns/note/scripts/generate_article.py` |
| スクリプト（記事投稿） | `.company/sns/note/scripts/post_to_note.py` |
| スクリプト（画像生成） | `.company/sns/note/scripts/generate_image.py` |
| cron用スクリプト | `.company/sns/note/daily_post.sh` |
| スタイル定義 | `.company/sns/note/references/styles/` |
| 記事出力先 | `.company/sns/note/output/{style}/{timestamp}/` |
| 投稿ログ | `.company/sns/note/logs/posts.log` |
| 競合分析・調査 | `.company/sns/note/research/` |
| 環境変数 | `.env`（プロジェクトルート） |
...
- このSKILL.md（`.company/skills/note-article/`）はスキル定義であり、データの保存先ではない
