...
| コア技術 | Anthropic API (SDK) + Playwright on クラウドコンテナ |
...
| 項目 | 決定事項 |
|------|---------|
| 方式 | **Anthropic API Key**（SDK経由） |
| 旧方式 | ~~Claude Code CLI（OAuth）~~ → コンテナ再ビルドでセッション切れ、ユーザーに再ログインさせるのは非現実的なため廃止 |
| 認証情報 | 環境変数 `ANTHROPIC_API_KEY` をコンテナに注入 |
| SDK | `anthropic` Python SDK（`pip install anthropic`） |
| バージョン管理 | Dockerfile内で `anthropic==x.x.x` をピン留め → CI/CDでイメージ再ビルド |

### 13.2 APIキー管理

| 方式 | メリット | デメリット |
|------|---------|-----------|
| **運営が1つのキーを管理** | ユーザーは何も設定不要、非技術者に優しい | 1ユーザーの暴走で全体に影響 |
| ユーザーに自分のキーを入れてもらう | コスト転嫁できる | 「非技術者ターゲット」と矛盾、離脱率UP |

**決定: 運営キー + プランごとのレートリミット**

- 運営がAPIキーを保持し、全ユーザーのコンテナに注入
- プラン別にメッセージ数/トークン数を制限（月次リセット）
- ユーザーは「自分のキーを使う」オプションも提供可（上級者向け）

### 13.3 コンテナアップデート

```
更新フロー:
  1. Dockerイメージを再ビルド（CI: GitHub Actions）
  2. レジストリにpush
  3. Fly Machines APIでローリングアップデート
     - ユーザーAのコンテナ停止 → 新イメージで起動 → ヘルスチェック → 次へ
  4. Volumeはそのまま引き継がれる（データ消えない）
  
ユーザー影響: なし（自動、通知不要）
ロールバック: 旧イメージタグを指定して再デプロイ
```

| 更新対象 | 頻度目安 | 方法 |
|---------|---------|------|
| Anthropic SDK | 月1回 | Dockerfile ピン留めバージョン変更 |
| Playwright | 月1回 | 同上 |
| Python / Node.js | 四半期 | ベースイメージ変更 |
| セキュリティパッチ | 随時 | 自動ビルド（Dependabot / Renovate） |
| アプリコード（Worker） | 随時 | git push → CI → デプロイ |

### 13.4 コスト管理・使用量トラッキング

| 項目 | 方法 |
|------|------|
| APIトークン消費量 | Anthropic API レスポンスの `usage` フィールドを記録 |
| ユーザー別集計 | `api_usage` テーブルに毎リクエスト記録（user_id, tokens_in, tokens_out, cost_usd） |
| プラン超過検知 | リクエスト前にカウンターチェック → 超過時は429返却 + UI通知 |
| ダッシュボード | 管理画面でユーザー別コスト一覧、異常検知アラート |
| 月次レポート | ユーザーに「今月の利用状況」メール（Stripe連携） |
...
| 障害 | 検知 | 自動復旧 | ユーザー通知 |
|------|------|---------|-------------|
| コンテナクラッシュ | Fly.io ヘルスチェック | 自動再起動（restart policy） | 不要（数秒で復旧） |
| Anthropic API 500 | レスポンスコード監視 | 指数バックオフで3回リトライ | 3回失敗後「一時的に処理が遅れています」 |
| Anthropic API レート制限 | 429レスポンス | リクエストキューイング + 遅延実行 | 「混雑中です。順番に処理します」 |
| コンテナVolume破損 | 起動時チェック | R2からデータ復元 | 「データを復元中です」 |
| DB接続断 | コネクションプールエラー | 再接続 + 一時ローカルキャッシュ | 不要（透過的に復旧） |
...
| データ | 保存先 | バックアップ方式 |
|--------|--------|----------------|
| ユーザー・社員設定 | Supabase PostgreSQL | Supabase自動バックアップ（日次） |
| 会話履歴 | コンテナ内Volume | **R2に日次同期**（cron） |
| SNS Cookie / Chrome Profile | コンテナ内Volume | R2に暗号化バックアップ |
| 生成ファイル | R2 | R2自体が永続（追加バックアップ不要） |
| APIログ・使用量 | PostgreSQL | Supabase自動バックアップに含まれる |

```
バックアップスケジュール:
  毎日 03:00 JST:
    - 全コンテナのVolume → R2へ差分同期
    - 会話履歴のスナップショット

  リストア手順:
    1. Fly Machines APIで新コンテナ起動
    2. R2からVolume復元
    3. ヘルスチェック通過で切り替え
```
...
| 監視項目 | ツール | アラート条件 |
|---------|--------|-------------|
| コンテナ稼働率 | Fly.io Metrics | 5分以上ダウン |
| API応答時間 | カスタムメトリクス | p95 > 10秒 |
| エラー率 | ログ集計 | 5xx > 5%/5分 |
| APIコスト | 使用量テーブル | 日次コストが前日比200%超 |
| ディスク使用量 | コンテナメトリクス | Volume 80%超 |

通知先: Slack（運営チーム） + メール（重大障害のみ）
