      ...
      <div className="w-full max-w-sm animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-[var(--color-border)] p-6 space-y-4">
          ...
          <button type="button" onClick={handleGoogleLogin}
            className="w-full py-2.5 bg-white border border-[var(--color-border)] text-sm font-medium text-[var(--color-text)] rounded-lg hover:bg-gray-50 transition-colors flex items-center justify-center gap-2">
        ...
        <p className="text-center text-sm text-[var(--color-subtext)] mt-4">
          アカウントをお持ちでない方は{" "}
          <Link href="/signup" className="text-violet-600 hover:underline font-medium">新規登録</Link>
        </p>
      </div>
    </div>
  );
